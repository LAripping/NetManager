<?php

include($_SERVER['DOCUMENT_ROOT'].'/about/sidebar.php');

$content = '<h2> Credits </h2>
			</br>
			<ul>
				<li>HTML template:</li>
				<ul style="list-style-type:none;">
					<li>Heavily edited by: Leonidas Tsaousis</li>
					<li>Original Designer: Bryant Smith</li>
					<li>email: <a href="mailto:templates@bryantsmith.com">
						templates@bryantsmith.com</a> </li>
					<li>Name: <i>Slick Red</i></li>
				</ul>
				<li>XML parsing functionality:</li>
				<ul style="list-style-type:none;">
					<li>Prototype implementation by: Tigran Tsaturyan</li>
					<li>Demonstrated in the free and available presentation 
					on <a href="http://www.slideshare.net/tigrantsaturyan58/network-packetstodatabase-v1"> slideshare</a>
					<li>Refactoring, addtitions, further tailoring by: Leonidas Tsaousis</li>
					<li>website: <a href="http://tigrantsat.me/">http://tigrantsat.me/</a></li>
				</ul>
				<li>Everything Else:</li>
				<ul style="list-style-type:none;">
					<li>including all PHP code, HTML views, CSS styling, MySQL queries etc.</li>
					<li>is the hard work of Leonidas Tsaousis</li>
					<li>email: <a href="mailto:sd1200184@di.uoa.gr">
						sd1200184@di.uoa.gr</a> </li>
					<li>The whole development process can be verified 
					</br>with a look at the projects remote repository on github. 
					</br>Just click on the avatar! &#187</li>
						<img style="position:absolute;left:750px;top:300px;width:150px;" 
							 src="https://avatars0.githubusercontent.com/u/19598145?v=3&s=466">					
					<a href="https://github.com/LAripping" style="background-image:https://avatars0.githubusercontent.com/u/19598145?v=3&s=466">
						<img style="position:absolute;left:750px;top:300px;width:150px;" 
						src="https://camo.githubusercontent.com/82b228a3648bf44fc1163ef44c62fcc60081495e/68747470733a2f2f73332e616d617a6f6e6177732e636f6d2f6769746875622f726962626f6e732f666f726b6d655f6c6566745f7265645f6161303030302e706e67"
						 alt="Fork me on GitHub" 
						data-canonical-src="https://s3.amazonaws.com/github/ribbons/forkme_left_red_aa0000.png">

					</a>
					
				</ul>
			</ul>';
	
include($_SERVER['DOCUMENT_ROOT'].'/theme/base.php')

?>
